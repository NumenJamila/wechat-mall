...
export default class util {
	...
	static getUserInfo(callback, thisArg) {
		console.log('core.os.isWechat()',core.os.isWechat())
		if (core.os.isWechat()) {
	...
	/**
	 * 授权手机号
	 * @param {authPhoneNumberCallback} callback 
	 * @param {*} thisArg 
	 */
	static authPhoneNumber(callback,url,thisArg){
		const key = '__USER__PHONE__';
		const info = wx.getStorageSync(key);
		if(info){
			callback.call(thisArg, info);
		}else{
			const getPhoneNumber = function (info) {
				listener.removeEventListener('user.get_phone', getPhoneNumber);
				if (info){
					wx.setStorageSync(key, info);
					callback.call(thisArg, info);
				} 
			};
			listener.addEventListener('user.get_phone', getPhoneNumber);
			if(url!=undefined){
				core.reLaunch({
					url: '/pages/user/tips-info/index?type=getPhoneNumber&url='+url,
				});
			}else{
				core.navigateTo({
					url: '/pages/user/tips-info/index?type=getPhoneNumber',
				});
			}
		}
	}
