...
export default class Form {
	...
	constructor(pageObj, name = 'form') {
		...
		pageObj.onFormBlurRegionInfo = pageObj.onFormBlurRegionInfo || this.onFormGroupChangeRegionInfo.bind(this);
		pageObj.onFormRegionChangeRegionInfo = pageObj.onFormRegionChangeRegionInfo || this.onFormGroupChangeRegionInfo.bind(this);
		pageObj.onFormRadioPickerChange = pageObj.onFormRadioPickerChange || this.onFormGroupChangeRegionInfo.bind(this);
		pageObj.onFormChooseLocation = pageObj.onFormChooseLocation || this.onFormGroupChangeRegionInfo.bind(this);
	...
	getSerializeValues() {
		...
		for (var i = 0; i < this.items.length; i++) {
			...
			if (item.type == 'checkbox') {
				...
				values[item.name] = item.value.join(' ');
			} else if(item.type == 'regioninfo'){
				console.log('item.value',item.value)
				item.value.region = item.value.region || [];
				for (let i = 0; i < item.value.region.length; i++) {
					values[item.name + '[' + i + ']'] = item.value.region[i];
				}
				if(item.value.region.length>0){
					values[item.name + '[' + 3 + ']'] = item.value.info;
				}
			} else {
				values[item.name] = item.value;
			}
	...
	onFormGroupChangeRegionInfo(e){
		...
		if (control.type == 'regioninfo') {
			if(type=='region'){
				control.value.region = value;
			}else{
				control.value.info = value;
			}
		}else if(control.type == 'radio' && control.options.col_show==5){
			control.value = control.options.items[value];
			this.pageObj.setData({pickerIndex:value});
		}else if(control.type == 'mylocation'){
			this.pageObj.biaoshi = 1;
			wx.chooseLocation({
					success: res => {
							console.log(res)
							control.value = res.address;
					},
					fail: res =>{
						wx.showToast({
							title: '请开启位置授权',
		...
		if (refresh && (type=='region' || control.type == 'radio')) {
			this.render();
		}
	}
    ...
    console.log('controlcontrolcontrol',control)
		...
		dg.chooseImage({
			...
			success: (res) => {
                ...
                console.log('controlcontrolcontrol2', control)
								this.render();
							} else {
								uploader(index + 1);
							}
