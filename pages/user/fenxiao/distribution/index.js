...
Page({
  ...
  load: function() {
    requestUtil.get(_DuoguanData.duoguan_host_api_url + '/index.php?s=/addon/DuoguanUser/DistributionApi/getApplyStatus.html', {}, (info) => {
      if (info.status == 200) {
        ...
        this.getDistributionApplyGoods();
  ...
  submit: function(e) {
    ...
    requestUtil.get(_DuoguanData.duoguan_host_api_url + '/index.php?s=/addon/DuoguanUser/DistributionApi/applyFenxiao.html', {
      data: e.detail.value
    }, (info) => {
      if (info == 1) {
        wx.showToast({
  ...
  onGetVerifyCodeTap: function(e) {
    ...
    requestUtil.get(_DuoguanData.duoguan_host_api_url + '/index.php?s=/addon/DuoguanUser/DistributionApi/getVerifyCode.html', {
      phone: tel
    }, (info) => {
      var that = this
      //倒计时
  ...
  getDistributionApplyGoods: function() {
    var that = this;
    requestUtil.get(_DuoguanData.duoguan_host_api_url + '/index.php?s=/addon/DuoguanUser/DistributionApi/getDistributionApplyGoods.html', {}, (info) => {
      that.setData({
        apply_goods: info
      });
    }, {});
  ...
  goods_buy_now: function (e) {
    ...
    requestUtil.get(_DuoguanData.duoguan_host_api_url + '/index.php?s=/addon/DuoguanShop/DuoguanShopApi/getGoodsInfo.html',
      { sid: goods_id, version: 200 },
      (data) => {
        requestUtil.get(_DuoguanData.duoguan_host_api_url + '/index.php?s=/addon/DuoguanShop/DuoguanShopApi/buyNow', {
          goods_id: goods_id, goods_number: 1, goods_attr: ''
        }, (info) => {
          if (info) {
            // requestUtil.pushFormId(e.detail.formId);
            wx.navigateTo({
              url: '/pages/shop/mallsure/mallsure?goods_id=' + info.goods_id + '&goods_number=' + info.goods_number + '&attr_str=' + info.attr_str + '&goods_attr=' + info.goods_attr + '&fx_apply_order=1'
            })
          } else {
            return false
          }
