...
Page({
    ...
    rechargeChange:function(){
        var that = this;
        that.setData({
            isFixedAmount: !that.data.isFixedAmount
        })
    },
    ...
    showModifyCardName:function(){
        this.setData({ show_modify_card_name: !this.data.show_modify_card_name})
    },

    modifyCardName:function(e){
        var that = this;
        that.showModifyCardName();
        requestUtil.get(API_URL + "/index.php?s=/addon/DuoguanUser/CardApi/modifyCardName.html", {name:e.detail.value.name}, (data) => {
            that.onPullDownRefresh();
        }, this, {});
    },
    rechargeShow:function(){
        this.setData({ recharge_show : !this.data.recharge_show});
    }
});
