...
Page({
    ...
    selectCommunityTabId:function(e){
        var that = this;
        ...
        wx.setStorage({ key: 'community_tab_id', data: id });
        that.selfAddressGoods(that.data.all_address.id, that.data.page, that.data.community_cate_id);
    },
    //读取首页数据
    onLoad: function(options) {
        ...
        var that = this
        wx.setStorage({ key: 'community_tab_id', data: that.data.community_cate_id });
        ...
        if (that.data.index_data.community_config && that.data.index_data.community_config.is_open == 1) {
            if (wx.getStorageSync('all_info')) {
                ...
                if (that.data.all_address) {
                    that.onShowBuildCommunityRelation(that.data.userInfo.uid, that.data.all_address.default_colonel_uid, that.data.all_address.id);
                    that.communityIndexBarrage(that.data.all_address.id);
                    requestUtil.get(_DuoguanData.duoguan_host_api_url + '/index.php?s=/addon/DuoguanShop/Api/selfAddressGoodsNew.html', {
                        address_id: that.data.all_address.id,
                        community_cate_id: that.data.community_cate_id,
                        version: 200,
                        community_tab_id: that.data.community_tab_id
                    }, (info) => {
                        that.setData({
                            address_goods: info.goods_list,
                            community_time: info.community_time
                        })
                        if ((info.goods_list == false || info.goods_list == '') && info.active_type == 1) {
                            wx.showModal({
                                title: '提示',
                        ...
                        if ((info.goods_list == false || info.goods_list == '') && info.active_type == 2) {
                            that.setData({
                                timeall: '',
                                seconds: ''
                            })
    ...
    onShareAppMessage: function() {
        ...
        if (that.data.index_data.community_config && that.data.index_data.community_config.is_open == 1) {
            return {
                ...
                path: 'pages/shop/mall/mall?other_uid=' + that.data.all_address.default_colonel_uid + '&log_other_uid=' + log_other_uid + '&shareuid=' + log_other_uid + '&sharetype=fx',
            }
        } else {
            return {
                title: that.data.shareInfo.title,
                path: 'pages/shop/mall/mall?log_other_uid=' + log_other_uid + '&shareuid=' + log_other_uid + '&sharetype=fx',
            }
        }
