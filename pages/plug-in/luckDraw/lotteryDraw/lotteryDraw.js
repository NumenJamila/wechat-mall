...
Page({
  ...
  onLoad: function(options) {
    ...
    var this_module = options.module || '';
    var this_uid = options.uid || 0;
    if (this_uid != 0) {
      let requesturl = _DuoguanData.duoguan_host_api_url + '/index.php/addon/MarketingLuckDraw/ApiShare/sharesIncNum';
      let requestData = {
        user_id: this_uid
      };
      requestUtil.get(requesturl, requestData, (res) => {
        console.log(res);
      });
    }
    this.setData({
  ...
  onPullDownRefresh: function() {
    requestUtil.get(_DuoguanData.duoguan_host_api_url + '/index.php/addon/MarketingLuckDraw/Api/getMarketInfo', {
      id: this.data.this_id
    }, (info) => {
      console.log(info)
      this.setData({
        this_market_data: info,
        this_jiangpin_data: info.jiangpin_data
      });
  ...
  onStartTap: function() {
    var that = this;
    var res_num = that.data.this_market_data.user_data.user_jiang_num;
    var is_jifen_open = that.data.this_market_data.is_jifen_open;

    if (res_num == 0 && is_jifen_open == 1) {
      wx.showModal({
        title: '提示',
        content: '是否消耗' + that.data.this_market_data.jifen_one_num + '积分抽1次奖',
        confirmText: '是',
        confirmColor: '#ff7104',
        cancelText: '算了',
        success(res) {
  ...
  onStartRequest: function() {
    ...
    this.startRQID = requestUtil.get(url, {
      id: this.data.this_id
    }, (info, _r, opts) => {
      opts.delay = 5500;
  ...
  onShareAppMessage: function() {
    let url = this.route + '?uid=' + this.data.this_market_data.uid + '&id=' + this.data.this_id + '&module=' + this.data.this_module;
    console.log(url);
    const {
      duoguan_app_is_superhome,
      duoguan_app_index_path
    } = _DuoguanData;
    if (duoguan_app_is_superhome) url = duoguan_app_index_path + '?_path=' + encodeURIComponent('/' + url);
    return {
