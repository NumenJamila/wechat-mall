...
Page({
  data: {
    isIphoneX:false,   //是否是苹果X系列
    goods_info: {
      info_data: [{ "name": "foot", "config": { "is_show": true } }, { "name": "swiper", "config": { "time": 1, "margin": 0 } }, { "name": "price", "config": { "margin": 0, "shop_price": 1, "market_price": 1, "g_number": 1, "sale_number": 1, "shar": 1 } }, { "name": "comment", "config": {} }, { "name": "sale", "config": { "margin": 0 } }, { "name": "attribute", "config": { "maigin": 0, "is_op": 0 } }, { "name": "detail", "config": { "margin": 0, "d_detail": 1, "d_comment": 1, "d_tit": "详情", "c_tit": "评价" } }, { "name": "icon", "config": {} }]
    },
    goods_specification: [],
    wxParseData: '',
    shop_config: [],
  ...
  onLoad: function (options) {
    ...
    var colonel_uid = 0;
    if (options.in_malldetail_type == 'vip'){
      wx.hideShareMenu();
      that.setData({ show_share_button:false});
    }
  ...
  initGoodsInfoData: function (data) {
    ...
    var product = {};
    product.item_code = data.id;
    product.title = data.g_name;
    product.desc = data.g_description;
    product.category_list = data.cate_arr;
    product.image_list = data.g_photo;
    var product_path;
    var log_other_uid = wx.getStorageSync("user_info").uid;
    if (that.data.community_config.is_open == 1) {
      var product_path = '/pages/shop/malldetail/malldetail?sid=' + data.id + '&shareuid=' + that.data.uid + '&self_address_id=' + that.data.community_config.self_address_id + '&colonel_uid=' + that.data.community_config.colonel_uid + '&log_other_uid=' + log_other_uid;
    } else {
      var product_path = '/pages/shop/malldetail/malldetail?sid=' + data.id + '&shareuid=' + that.data.uid + '&log_other_uid=' + log_other_uid;
      if (that.data.share_type == 'miaosha') product_path = '/pages/shop/malldetail/malldetail?sid=' + data.id + '&shareuid=' + that.data.uid;
    }
    product.src_mini_program_path = product_path;
    var is_autoplay = true;
    if (data.g_video && data.g_video.v_path) is_autoplay = false;
    var price_config = {};
    for (i = 0; i < data.info_data.length; i++){
      if (data.info_data[i].name == 'price') price_config = data.info_data[i];
    }
    that.setData({
      goods_info: data,
  ...
  * */
  ...
  onShareAppMessage: function () {
    ...
    if (that.data.community_config.is_open == 1) {
      ...
      return {
        title: '【' + that.data.goods_show_shop_price + '元】' + that.data.goods_info.g_name,
        desc: '',
        path: path
      };
    } else {
