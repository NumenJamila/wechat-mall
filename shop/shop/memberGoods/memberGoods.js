...
Page({
  ...
  getVipSpecialGoods:function(){
    var that = this;
    if (that.data.special_page > 1) {
      wx.showLoading({
        title: '加载中',
        mask: true
      });
    ...
    requestUtil.get(_DuoguanData.duoguan_host_api_url + '/index.php?s=/addon/DuoguanShop/UserApi/getVipSpecialGoods', { page: that.data.special_page }, (data) => {
      if (data == null || data == '') {
        that.setData({
          special_load_more: false
        })
      } else if (data.length < 5) {
        var special_goods = that.data.special_goods.concat(data);
        that.setData({
        ...
        that.setData({
          special_goods: special_goods,
          special_page: that.data.special_page + 1,
          special_load_more: true
        })
      }
    }, this, { isShowLoading: false });
  },
  ...
  goods_detail: function (e) {
    ...
    util.trySyncUserInfo(function () {
      wx.navigateTo({
        url: '/pages/shop/malldetail/malldetail?sid=' + e.currentTarget.id + '&in_malldetail_type=vip',
      })
    })
  },
