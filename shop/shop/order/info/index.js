...
Page({
    ...
    //分包裹查看物流
    new_wuliu_info_bind: function (e) {
        var that = this;
        var express_code = e.currentTarget.dataset.express_code;
        if (express_code == '' || express_code == null) {
            wx.showModal({
                title: '提示',
                content: '对不起，该订单暂无物流信息',
                showCancel: false
            ...
            wx.navigateTo({
                url: '../../../shop/mallwuliu/index?oid=' + that.data.oinfo.id + '&express_code=' + express_code
            })
        }
    },
    ...
    order_go_refund: function(e){
        var og_id = e.currentTarget.dataset.og_id;
        if (!og_id) og_id = 0;
        wx.navigateTo({
            url: '/pages/shop/applyRefund/applyRefund?oid=' + e.target.dataset.id + '&og_id=' + og_id,
        })
    },
    refund_go_detail:function(e){
        var tid = e.currentTarget.dataset.tid;
        wx.navigateTo({
            url: '/pages/shop/refundDetail/refundDetail?rid=' + this.data.oinfo.rid + '&tid=' + tid,
        })
    },
    //电话
    ...
    bindErrandsPhone: function () {
        var that = this
        wx.makePhoneCall({
            phoneNumber: that.data.oinfo.delivery_clerk_config.phone
        })
    },
