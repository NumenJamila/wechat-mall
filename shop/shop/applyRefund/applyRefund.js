...
Page({
  ...
  loaddata: function () {
    ...
    if (that.data.this_og_id > 0){
      requestUtil.get(_DuoguanData.duoguan_host_api_url + '/index.php?s=/addon/DuoguanShop/Api/getGoodsMaxRefund.html',
        { oid: that.data.this_order_id, og_id: that.data.this_og_id },
        (data) => {
          that.setData({ goods_refund_info: data })
        }, this, { isShowLoading: false });
    }
  },
  ...
  formSubmit: function (e) {
    ...
    requestUtil.get(_DuoguanData.duoguan_host_api_url + '/index.php?s=/addon/DuoguanShop/Api/postRefundReason.html',
      { oid: that.data.this_order_id, uid: that.data.order_info.user_id, content: t_con, refund_type: that.data.index, formId: e.detail.formId, max_refund: that.data.order_info.refund_amount, mobile: e.detail.value.mobile, og_id: that.data.this_og_id },
      (data) => {
        console.log(data)
        if (data.code == 1) {
  ...
  uploadImage: function (filePathList, index, callback, id) { //上传图片
    if (filePathList.length == index) { // 递归出口
      ...
      let url = _DuoguanData.duoguan_host_api_url + '/index.php/addon/DuoguanShop/Api/uploadImage/rid/' + id;
