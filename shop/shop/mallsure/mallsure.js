    ...
    select_address_bind: function() {
        ...
        util.chooseAddress(function(res) {
            ...
            if (res.wxAddress != undefined) {
                ...
                that.data.post_data.wxAddress.latitude = res.latitude
                that.data.post_data.wxAddress.longitude = res.longitude
            }
            that.setData({
                post_data: that.data.post_data
            })
        ...
        requestUtil.get(_DuoguanData.duoguan_host_api_url + '/index.php?s=/addon/DuoguanShop/DuoguanShopApi/makePay.html', {
            ...
            if (info === '零元商品支付成功') {
                ...
                if (info == '余额支付成功') {
                    that.importPurchaseOrder(that.data.order_id);
                    wx.redirectTo({
                        url: '../pay-success/pay-success?order_id=' + that.data.order_id+'&is_yue=1',
                    });
                ...
                if (info == '货到付款设置成功') {
                    that.importPurchaseOrder(that.data.order_id);
                    wx.redirectTo({
                        url: '../pay-success/pay-success?order_id=' + that.data.order_id + '&is_yue=2',
                    });
                }
            }
